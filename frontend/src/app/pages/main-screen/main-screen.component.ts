...
export class MainScreenComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.search$.pipe(debounceTime(250), distinctUntilChanged(), switchMap(q => this.booksApi.listBooks(q, this.difficulty)), takeUntil(this.destroy$)).subscribe({ next: b => this.books = b, error: () => this.snack.open('Unable to load adventures.', 'Close', {duration:3000}) });
    this.search$.next('');
  }
  search(query: string): void { this.search$.next(query); }
  filter(difficulty: string): void { this.difficulty = difficulty; this.search$.next(''); }
